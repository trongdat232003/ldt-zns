const { getCustomerPhone, sendZNSForInvoice } = require("../simpleReminder");
const {
...
async function sendDueReminders() {
  ...
  for (let i = 0; i < dueToday.length; i++) {
    ...
    console.log(
      `    Customer: ${reminder.customer_name} (${reminder.customer_code})`,
    );
    console.log(`    Purchase: ${reminder.purchase_date}`);
    console.log(`    Due: ${reminder.due_date}`);

    // Lấy SĐT từ reminder; nếu chưa có thì truy vấn lại bằng customer_id
    let phone = reminder.phone;

    if (!phone && reminder.customer_id) {
      console.log(
        `    🔎 Phone missing, fetching from customer ID: ${reminder.customer_id}`,
      );
      phone = await getCustomerPhone(reminder.customer_id);
    }
